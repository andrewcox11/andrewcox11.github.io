...
Q1b
```{r}
summary_df <- NFL2022_stuffs %>% filter(wp > 0.2 & wp < 0.75 & down <= 2 & half_seconds_remaining > 120) %>% group_by(posteam) %>% summarise(mean_pass = mean(pass, na.rm = TRUE))
...
Q1c
```{r}
Ggplot(data=NFL2022_stuffs)+ Geom_point(mapping=aes(x=percentage of pass plays, y=Team with possession))
